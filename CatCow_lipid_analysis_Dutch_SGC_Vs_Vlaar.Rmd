---
title: "CatCow Lipid Analysis - Dutch SGC vs Vlaardingen"
author: "LW"
date: "2024-11-20"
...
gs4_deauth()
Data <-read_sheet(ss="https://docs.google.com/spreadsheets/d/121lVA7w2d5MKc5Vvm4d0aR5vaLV4ChfMM-Eh2qYxYJA/edit#gid=793877156", sheet="Dutch summary sheet for R")
...
Plot <-ggplot() +
  ...
  scale_x_continuous(position="top", limits=c(-38,-20))+
  scale_y_continuous(position = "right",limits=c(-38,-20))+
  theme_bw()+
...
p <- Plot + geom_point(data=Data, aes(y=d13C18,x=d13C16, fill = Phase), colour="black", alpha =0.7, pch=21, size=2)+
  scale_fill_manual(name="Phase", breaks=c("SGC","Vlaardingen"), values=c("#0072B2","#E69F00"), labels=c("Single Grave Culture", "Vlaardingen"))+
...
p <- Plot + geom_point(data=Data, aes(y=d13C18,x=d13C16, fill = Site), colour="black", alpha =0.7, pch=21, size=2)+
  scale_fill_manual(name="Site", breaks=c("Keinsmerbrug","Mienakker","Zeewijk","Vlaardingen","Steynhof"), values=c("#000000","#999999","#56B4E9","#F0E442","#D55E00"), labels=c("Keinsmerbrug", "Mienakker", "Zeewijk","Vlaardingen","Steynhof"))+
  theme(legend.text=element_text(size=12))+
  #theme(legend.position = c(0.7, 0.2))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p
```
display by sample type

```{r}

p <- Plot + geom_point(data=Data, aes(y=d13C18,x=d13C16, fill = Sample_type), colour="black", alpha =0.7, pch=21, size=2)+
  scale_fill_manual(name="Site", breaks=c("matrix","food crust"), values=c("white","#000000"), labels=c("Matrix","Food crust"))+
  theme(legend.text=element_text(size=12))+
  #theme(legend.position = c(0.7, 0.2))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p
...
plot_bigD <- plot_curv + geom_point(data= Data, aes(x = d13C16, y = bigD, fill = Phase), colour="black", alpha =0.7, pch=21, size=3) + 
        scale_fill_manual(values=c("#0072B2","#E69F00")) + #colores del Env
...
plot_bigD <- plot_curv + geom_point(data= Data, aes(x = d13C16, y = bigD, fill = Site), colour="black", alpha =0.7, pch=21, size=3) + 
        scale_fill_manual(name="Site", breaks=c("Keinsmerbrug","Mienakker","Zeewijk","Vlaardingen","Steynhof"), values=c("#000000","#999999","#56B4E9","#F0E442","#D55E00"), labels=c("Keinsmerbrug","Mienakker","Zeewijk","Vlaardingen","Steynhof"))+
  #scale_fill_manual(values=c("#000000","#999999","#56B4E9","#F0E442","#D55E00")) + #colores del Env
        #scale_starshape_manual(values=c(1, 5, 11, 23, 29)) +  #formas Use
        geom_abline(intercept=-1 ,slope= 1, linetype="dashed") 
