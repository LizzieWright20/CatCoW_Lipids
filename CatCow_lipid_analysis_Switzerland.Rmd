...
https://docs.google.com/spreadsheets/d/121lVA7w2d5MKc5Vvm4d0aR5vaLV4ChfMM-Eh2qYxYJA/edit#gid=982841462
...
gs4_deauth()
Data <-read_sheet(ss="https://docs.google.com/spreadsheets/d/121lVA7w2d5MKc5Vvm4d0aR5vaLV4ChfMM-Eh2qYxYJA/edit#gid=982841462", sheet="Summary Sheet for R")
...
However, we would need to think carefully about which ones to use based on the zooarch data. For example, marine is probably not useful in this context. But if you create your own sheet you can upload it in the script below. 
...
Plot <-ggplot() +
  ...
  scale_x_continuous(position="top", limits=c(-40,-20))+
  scale_y_continuous(position = "right",limits=c(-40,-20))+
  theme_bw()+
  ...
  annotate("text", label = "Marine", x = -23, y = -20, size = 3, colour = "black")+ 
  annotate("text", label = "Porcine", x = -27, y = -24, size = 3, colour = "black")+ 
  annotate("text", label = "Ruminant \n adipose", x = -26, y = -29, size = 3, colour = "black")+ 
  annotate("text", label = "Dairy", x = -27, y = -35, size = 3, colour = "black")+
  annotate("text", label = "Plant", x = -35, y = -32, size = 3, colour = "black")+
...
p <- Plot + geom_point(data=Data, aes(y=d13C18,x=d13C16, fill = Phase), colour="black", alpha =0.7, pch=21, size=2)+
  scale_fill_manual(name="Typology/phase", breaks=c("Corded Ware","Horgen"), values=c("#D55E00","#009E73"), labels=c("Corded ware pots", "Horgen pots"))+
...
```{r}
p <- Plot + geom_point(data=Data, aes(y=d13C18,x=d13C16, fill = Phase, size = Concentration), colour="black", alpha =0.7, pch=21)+
  scale_fill_manual(name="Typology/phase", breaks=c("Corded Ware","Horgen"), values=c("#D55E00","#009E73"), labels=c("Corded ware pots", "Horgen pots"))+
  theme(legend.text=element_text(size=12))+
  #theme(legend.position = c(0.7, 0.2))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
Interesting that low lipid concentrations seem to plot in the -30, -30 area. Could this be contamianation or reflect plants in these pots. Need to look at  APAA data. 


Now create a bigD plot
...
```{r}
plot_bigD <- plot_curv + geom_point(data= Data, aes(x = d13C16, y = bigD, fill = Phase), colour="black", alpha =0.7, pch=21, size=3) + 
        scale_fill_manual(values=c("#D55E00","#009E73")) + #colores del Env
